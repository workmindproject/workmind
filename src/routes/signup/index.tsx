import { component$, useStore } from "@builder.io/qwik";
import type { DocumentHead } from "@builder.io/qwik-city";
import ApiSignup from "~/components/signup.form/api.signup";
import EmailSignupForm from "~/components/signup.form/email.signup-form";
import PasswordSignupForm from "~/components/signup.form/password.signup-form";

export enum SIGNIN_STATE {
    'email',
    'password',
    'api'
}

export default component$(() => {
    const config = {
        // mode: 'bg-white dark:bg-gray-900' 
        mode: ''
    }
    const store = useStore({ step: SIGNIN_STATE['email'], email: '', password: '' })

    return (
        <div class="container px-6 py-12">
            <div class="flex flex-col justify-center items-center w-full max-w-3xl p-8 mx-auto lg:px-12 lg:w-3/5">
                <div class="w-full">
                    <h1 class="text-2xl font-semibold tracking-wider capitalize">
                        Get your free account now.
                    </h1>

                    <p class="mt-4">
                        Let’s get you all set up so you can verify your personal account and begin setting up your profile.
                    </p>
                    {
                        store.step === SIGNIN_STATE['email']
                            ? <EmailSignupForm onValue$={(e, v) => {
                                store.step = SIGNIN_STATE['password'];
                                store.email = v as string;
                            }} />
                            : store.step === SIGNIN_STATE['password']
                                ? <PasswordSignupForm email={store.email} onValue$={(e, v) => {
                                    store.password = v as string;
                                    store.step = SIGNIN_STATE['api'];
                                }} />
                                : store.step === SIGNIN_STATE['api']
                                    ? <ApiSignup email={store.email} password={store.password}
                                        onValid$={(v) => {
                                            store.password = v as string;
                                            store.step = SIGNIN_STATE['api'];
                                        }}
                                        onInvalid$={(v) => {
                                            store.password = v as string;
                                            store.step = SIGNIN_STATE['api'];
                                            window.location.href = '/account';
                                        }}></ApiSignup>
                                    : <></>

                    }
                    <div class="flex justify-center items-center mt-6">
                        <a
                            href="/signin"
                            target="_blank"
                            class="inline-flex items-center text-gray-700 font-medium text-xs text-center " >
                            <span class="ml-2" >
                                You have an account?
                                <a href="signin" class="text-xs ml-2 text-blue-500 font-semibold" >
                                    Signin here
                                </a>
                            </span>
                        </a>
                    </div>
                    {/* <div class="mt-6">
                            <h1 class="text-gray-500 dark:text-gray-300">Select type of account</h1>

                            <div class="mt-3 md:flex md:items-center md:-mx-2">
                                <button class="flex justify-center w-full px-6 py-3 text-white bg-blue-500 rounded-md md:w-auto md:mx-2 focus:outline-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                    </svg>

                                    <span class="mx-2">
                                        client
                                    </span>
                                </button>

                                <button class="flex justify-center w-full px-6 py-3 mt-4 text-blue-500 border border-blue-500 rounded-md md:mt-0 md:w-auto md:mx-2 dark:border-blue-400 dark:text-blue-400 focus:outline-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>

                                    <span class="mx-2">
                                        worker
                                    </span>
                                </button>
                            </div>
                        </div> */}

                </div>
            </div>
        </div>
    );
});

export const head: DocumentHead = {
    title: 'Signup'
}